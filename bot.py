...
def main_kb(user_id):
    ...
    kb.add(types.KeyboardButton(f"💎 {price} ⭐ Stars tolab kirish"))
    kb.add(types.KeyboardButton("⭐ Stars qanday olaman?"))
    kb.add(
...
@bot.message_handler(func=lambda m: m.text and (m.text.startswith("⭐ Stars") or m.text.startswith("💎")))
def pay_handler(msg):
...
@bot.message_handler(func=lambda m: m.text in ["ℹ️ Stars qanday olaman?", "⭐ Stars qanday olaman?"])
def stars_info(msg):
