...
def set_member(user_id, stars):
    ...
    conn = get_db()
    conn.execute("""
        UPDATE users SET is_member=1, expires_at=?, total_paid=total_paid+?
        WHERE user_id=?
    """, (expires, stars, user_id))
    conn.commit()
    conn.close()
...
def is_active_member(user_id):
    ...
    if not u or not u["is_member"]:
        return False
    if u["expires_at"] and datetime.fromisoformat(u["expires_at"]) < datetime.now():
        conn = get_db()
        conn.execute("UPDATE users SET is_member=0 WHERE user_id=?", (user_id,))
        conn.commit()
        conn.close()
        return False
    return True
...
def main_kb(user_id):
    ...
    kb.add(
        types.KeyboardButton(f"⭐ Stars {price} tolab kirish"),
        types.KeyboardButton("ℹ️ Stars qanday olaman?")
    )
    kb.add(
...
@bot.message_handler(func=lambda m: m.text and m.text.startswith("⭐ Stars"))
def pay_handler(msg):
    ...
    bot.send_invoice(
        chat_id=msg.chat.id,
        title="⭐ VIP A'zolik",
        description=f"30 kunlik VIP a'zolik. Barcha maxsus kontentga kirish imkoniyati.",
        payload="vip_membership",
        provider_token="",
        currency="XTR",
        prices=[LabeledPrice("VIP A'zolik (30 kun)", price)],
        start_parameter="pay"
    )
...
@bot.message_handler(func=lambda m: m.text == "ℹ️ Stars qanday olaman?")
def stars_info(msg):
    text = (
        "⭐ <b>Telegram Stars qanday olish mumkin?</b>\n\n"
        "1️⃣ <b>Telegram ilovasi orqali:</b>\n"
        "   • Sozlamalar → Telegram Stars → Stars sotib olish\n\n"
        "2️⃣ <b>App Store / Google Play:</b>\n"
        "   • To'g'ridan-to'g'ri telefon do'koni orqali\n\n"
        "3️⃣ <b>Fragment.com orqali:</b>\n"
        "   • fragment.com saytiga kiring\n"
        "   • Stars sotib oling (TON bilan)\n\n"
        "💡 <b>Eslatma:</b> Stars sotib olgach, bot'ga qaytib\n"
        "«⭐ Stars to'lab kirish» tugmasini bosing!"
    )
    bot.send_message(msg.chat.id, text, parse_mode="HTML", reply_markup=main_kb(msg.from_user.id))
...
def help_handler(msg):
    text = (
        ...
        "   → Havola orqali kanalga kiring: " + CHANNEL_INVITE + "\n\n"
        "🔹 <b>A'zolik qancha vaqtga?</b>\n"
        "   → 30 kun\n\n"
        "🔹 <b>Stars qaytariladi?</b>\n"
        "   → Xarid qilingan Stars qaytarilmaydi\n\n"
        "🔹 <b>Profilimni qanday ko'raman?</b>\n"
        "   → «👤 Profilim» tugmasini bosing"
    )
