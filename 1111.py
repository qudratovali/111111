...
def main_kb(user_id):
    ...
    kb.add(
        types.KeyboardButton(f"🔒 VIP kirish — {price} ⭐"),
    ...
    kb.add(types.KeyboardButton("❓ Savollar"))
...
@bot.message_handler(func=lambda m: m.text and m.text.startswith("🔒 VIP kirish"))
def pay_handler(msg):
    ...
    bot.send_invoice(
        chat_id=msg.chat.id,
        title="⭐ VIP A'zolik",
        description=f"Doimiy VIP a'zolik. Barcha maxsus kontentga kirish imkoniyati.",
...
def stars_info(msg):
    text = (
        "💳 <b>Stars qanday sotib olish mumkin?</b>\n\n"
        ...
        "💡 Stars sotib olgach, botga qaytib\n"
        "«🔒 VIP kirish» tugmasini bosing!"
    )
    bot.send_message(msg.chat.id, text, parse_mode="HTML", reply_markup=main_kb(msg.from_user.id))
...
def faq_handler(msg):
    text = (
        ...
        f"   → Havola orqali kanalga kiring: {CHANNEL_INVITE}\n\n"
