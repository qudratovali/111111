...
def main_kb(user_id):
    ...
    kb.add(types.KeyboardButton(f"🔓 VIP kirish — {price} ⭐"))
    kb.add(
...
@bot.message_handler(func=lambda m: m.text in ["👤 Profilim", "👤 Mening profilim"])
...
@bot.message_handler(func=lambda m: m.text and (m.text.startswith("⭐ Stars") or m.text.startswith("💎") or m.text.startswith("🔓 VIP")))
def pay_handler(msg):
...
@bot.message_handler(func=lambda m: m.text in ["ℹ️ Stars qanday olaman?", "⭐ Stars qanday olaman?", "💳 Stars olish"])
def stars_info(msg):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("⭐ Stars sotib olish", url="https://t.me/tezstar_bot/app?startapp=7514690928"))
    text = (
        "⭐ <b>Stars qanday olish mumkin?</b>\n\n"
        "Quyidagi tugmani bosib @tezstar_bot orqali\n"
        "Stars sotib oling — tez va qulay! 👇"
    )
    bot.send_message(msg.chat.id, text, parse_mode="HTML", reply_markup=kb)

@bot.message_handler(func=lambda m: m.text in ["🆘 Yordam", "📞 Yordam", "❓ Savollar"])
